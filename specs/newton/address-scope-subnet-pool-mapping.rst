...
#. A l3_policy is created with an ip_pool, ip_version, and
   subnet_prefix_length (IPv4 only), but no address_scope or subnetpool(s) are
   provided for that address family. There are two possibilities in this
   scenario:

     *  An implicit subnetpool exists for the given for an address family
        enabled by ip_version. In this case the implicit subnetpool and its
        address scope are associated with the l3_policy and any supplied
